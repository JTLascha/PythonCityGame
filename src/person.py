...
class Family:
    ...
    def marriage(self, family1, family2):
        pass
...
class Person:
    ...
    def __init__(self, age, job, job_location, home_location, index, ww):
        ...
        self.QoL = 50
        self.skills = []
        for i in range(0,Skills.total):
            self.skills.append(10)	# skills should be randomized. Add that later. For now they're all set to 10.
        all.append(self)
    ...
    def updateQoL(self,child_modifier=-1):
        newQoL = self.QoL
        if child_modifier > -1:
            newQoL = newQoL * 0.8 + child_modifier * 0.2
